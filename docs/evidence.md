...
| 주장 | 시나리오 | 기대 결과 | 대상 테스트 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 잘못된 비밀번호가 반복되면 계정이 잠긴다 | username 기준 30분 안에 로그인 5회 실패 | 423 Locked와 DB 계정 잠금 | `AuthServiceImplTest.login_locksUser_afterFiveFailures` | TODO | Redis 계정 잠금 카운터와 DB lock 로직 필요 |
