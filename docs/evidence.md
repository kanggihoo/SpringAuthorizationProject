...
발표 및 포트폴리오용 해설 문서: [Phase 3 - Refresh Token Rotation & Reuse Detection](portfolio/phase3-refresh-token-rotation-reuse-detection.md)

| 주장 | 시나리오 | 기대 결과 | 대상 테스트 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| Refresh Token은 사용 시 회전된다 | 유효한 Refresh Token이 한 번 사용된다 | 새 Refresh Token이 기존 저장 토큰을 대체한다 | `TokenLifecycleServiceImplTest.rotate_replacesActiveRefreshToken` | PASS | 2026-05-26에 `./gradlew.bat test --tests org.example.security.token.TokenLifecycleServiceImplTest --tests org.example.service.AuthServiceImplTest --tests org.example.controller.AuthControllerTest`로 검증 |
| 이전 Refresh Token은 재사용할 수 없다 | 이전 토큰이 저장된 토큰과 다르다 | `REFRESH_TOKEN_REUSED` 인증 실패 | `TokenLifecycleServiceImplTest.rotate_rejectsReusedRefreshToken` | PASS | 2026-05-26에 `./gradlew.bat test --tests org.example.security.token.TokenLifecycleServiceImplTest --tests org.example.service.AuthServiceImplTest --tests org.example.controller.AuthControllerTest`로 검증 |

## Phase 4 - Account Lock & Admin Recovery

발표 및 포트폴리오용 해설 문서: [Phase 4 - Account Lock & Admin Recovery](portfolio/phase4-account-lock-admin-recovery.md)

| 주장 | 시나리오 | 기대 결과 | 대상 테스트 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 잘못된 비밀번호가 반복되면 계정이 잠긴다 | username 기준 30분 안에 로그인 5회 실패 | 423 Locked와 DB 계정 잠금 | `AuthServiceImplTest.login_locksUser_afterFiveFailures` | PASS | 2026-05-27에 Phase 4 focused command run(`rtk proxy ./gradlew.bat test --tests org.example.service.AuthServiceImplTest --tests org.example.security.jwt.JwtAuthenticationFilterTest --tests org.example.service.AdminServiceImplTest --tests org.example.controller.AdminControllerSecurityTest`)으로 검증 |
| 잠긴 계정은 로그인할 수 없다 | 잠금 후 올바른 비밀번호로 로그인 | 423 Locked | `AuthServiceImplTest.login_throwsLockedException_whenUserLocked` | PASS | 2026-05-27에 Phase 4 focused command run(`rtk proxy ./gradlew.bat test --tests org.example.service.AuthServiceImplTest --tests org.example.security.jwt.JwtAuthenticationFilterTest --tests org.example.service.AdminServiceImplTest --tests org.example.controller.AdminControllerSecurityTest`)으로 검증 |
| 잠긴 계정은 기존 Access Token을 사용할 수 없다 | 잠금 후 기존 토큰으로 Protected API 호출 | `SecurityContext`가 인증 상태가 되지 않는다 | `JwtAuthenticationFilterTest.doFilter_doesNotAuthenticate_whenUserIsLocked` | PASS | 2026-05-27에 Phase 4 focused command run(`rtk proxy ./gradlew.bat test --tests org.example.service.AuthServiceImplTest --tests org.example.security.jwt.JwtAuthenticationFilterTest --tests org.example.service.AdminServiceImplTest --tests org.example.controller.AdminControllerSecurityTest`)으로 검증 |
| ADMIN은 계정 잠금을 해제할 수 있다 | ADMIN이 사용자의 잠금을 해제한다 | 계정 잠금이 해제된다 | `AdminServiceImplTest.unlockUser_unlocksLockedUser` | PASS | 2026-05-27에 Phase 4 focused command run(`rtk proxy ./gradlew.bat test --tests org.example.service.AuthServiceImplTest --tests org.example.security.jwt.JwtAuthenticationFilterTest --tests org.example.service.AdminServiceImplTest --tests org.example.controller.AdminControllerSecurityTest`)으로 검증 |
| USER는 계정 잠금을 해제할 수 없다 | USER가 admin endpoint를 호출한다 | 403 Forbidden | `AdminControllerSecurityIntegrationTest.userCannotUnlockAccount_withProductionSecurityConfig` | PASS | 2026-05-27에 production `SecurityConfig` 기반 command run(`rtk proxy ./gradlew.bat test --tests org.example.controller.AdminControllerSecurityIntegrationTest`)으로 검증 |

Phase 4 implementation policy:

- Account Lock counting uses Redis key `auth:login:fail:user:{username}`.
- The account counter window is 30 minutes. On the first failure, set TTL; on the fifth failure within that window, lock the User.
- IP-based abuse throttling is a follow-up hardening item, not a Phase 4 completion requirement. If added later, it should use a separate key such as `auth:login:fail:ip-user:{ip}:{username}` for temporary throttling, not final Account Lock state.
- Redis counters are short-lived failure counters. Final Account Lock state is stored in DB with `users.accountNonLocked=false`.
- ADMIN unlock clears DB Account Lock and deletes `auth:login:fail:user:{username}`. IP-specific throttling keys may expire by TTL.
- Failed login responses must not expose remaining attempts. Unknown User and wrong password should use the same credential failure response.
- Gmail-based self-service unlock is a follow-up hardening item, not a Phase 4 completion requirement. Phase 4 completion requires ADMIN unlock evidence.
...
| 주장 | 시나리오 | 기대 결과 | 대상 테스트 | 상태 | 비고 |
| --- | --- | --- | --- | --- | --- |
| USER는 ADMIN API에 접근할 수 없다 | USER가 `/admin/**`를 호출한다 | 403 Forbidden | `AdminControllerSecurityIntegrationTest.userCannotUnlockAccount_withProductionSecurityConfig` | PASS | 2026-05-27에 production `SecurityConfig` 기반 command run(`rtk proxy ./gradlew.bat test --tests org.example.controller.AdminControllerSecurityIntegrationTest`)으로 검증 |
| ADMIN은 ADMIN API에 접근할 수 있다 | ADMIN이 `/admin/**`를 호출한다 | 200 OK | `AdminControllerSecurityIntegrationTest.adminCanUnlockAccount_withProductionSecurityConfig` | PASS | 2026-05-27에 production `SecurityConfig` 기반 command run(`rtk proxy ./gradlew.bat test --tests org.example.controller.AdminControllerSecurityIntegrationTest`)으로 검증 |
