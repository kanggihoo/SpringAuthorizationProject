...
| USER는 계정 잠금을 해제할 수 없다 | USER가 admin endpoint를 호출한다 | 403 Forbidden | `AdminControllerSecurityTest.userCannotUnlockAccount` | PASS | 2026-05-27에 Phase 4 focused command run(`rtk proxy ./gradlew.bat test --tests org.example.service.AuthServiceImplTest --tests org.example.security.jwt.JwtAuthenticationFilterTest --tests org.example.service.AdminServiceImplTest --tests org.example.controller.AdminControllerSecurityTest`)으로 검증 |
